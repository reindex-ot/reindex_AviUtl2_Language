...
>
> 日本語以外の言語の場合は時間制御メニューの`等速移動`、`加速移動`、`減速移動`、`加減速移動`のテキストが未だに変更できません。
> こちらの問題はテキストの置換での対処ができないため、仕様と思うしかないかもしれません。
>
> ソースとなる公式の英語言語ファイルには時折、「テキストの欠落の問題」を抱えている状態の場合があります。<br>
> テキストが欠落している場合、その部分だけ**日本語のままになる問題**が発生します。<br>
> 自分が公開しているファイルでは極力、欠落問題に当たらないようにできるだけ確認と修正をしています。
>
> もしも欠落した箇所を見つけた場合は Issue でご報告ください。
